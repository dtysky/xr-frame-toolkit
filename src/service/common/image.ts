...
export async function decodeImage(src: string): Promise<IImage> {
  ...
  if (ext === '.exr') {
    ...
    // const buffer = instance.getBytes();
    const buffer = instance.getBytes().slice();

    return {
      name,
      width, height, premultiplyAlpha: false,
      hdr: true, rgb: false, buffer
    }
  }
  ...
  } else if ((info as any).space !== 'rgb') {
    throw new Error(`图片色彩空间为'${info.space}'，不支持，仅支持'rgb'和'srgb'！`);
  }
  
  return {
    name,
    width: info.width!, height: info.height!, premultiplyAlpha: (info as any).premultiplied!,
